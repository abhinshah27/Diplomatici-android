...
import rs.highlande.app.diplomatici.fcm.FCMNotNeededException;
...
import rs.highlande.app.diplomatici.features.profile.SelectIdentityActivity;
import rs.highlande.app.diplomatici.features.viewers.WebViewActivity;
import rs.highlande.app.diplomatici.features.viewers.WebViewActivityDocuments;
...
import rs.highlande.app.diplomatici.widgets.HLCustomTypefaceSpan;
...
//			Intent intent = new Intent(Intent.ACTION_VIEW);
	...
	public static void fireMapIntent(Context context, double latitude, double longitude, @Nullable String address) {
		if (isContextValid(context)) {
			String sUri = "geo:" + latitude + "," + longitude;
...
//								LogUtils.i(context.getClass().getCanonicalName(), "Refreshed FCM token: " + refreshedToken);
								...
								if (!DiplomaticiApp.fcmTokenSent || (Utils.isStringValid(refreshedToken) && !fcm.equals(refreshedToken))) {
	...
	public static void logOut(Activity activity, @Nullable MaterialDialog dialog) {
		...
		Realm.deleteRealm(DiplomaticiApp.realmConfig);
