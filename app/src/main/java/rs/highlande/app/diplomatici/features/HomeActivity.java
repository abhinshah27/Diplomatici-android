...
package rs.highlande.app.diplomatici.features;
...
import rs.highlande.app.diplomatici.base.BasicInteractionListener;
import rs.highlande.app.diplomatici.base.DiplomaticiApp;
import rs.highlande.app.diplomatici.base.HLActivity;
import rs.highlande.app.diplomatici.base.OnBackPressedListener;
...
import rs.highlande.app.diplomatici.connection.realTime.RealTimeCommunicationHelperKotlin;
import rs.highlande.app.diplomatici.fcm.SendFCMTokenService;
import rs.highlande.app.diplomatici.features.chat.ChatRoomsFragment;
import rs.highlande.app.diplomatici.features.createPost.CreatePostActivityMod;
import rs.highlande.app.diplomatici.features.globalSearch.GlobalSearchFragment;
import rs.highlande.app.diplomatici.features.notifications.NotificationAndRequestHelper;
import rs.highlande.app.diplomatici.features.notifications.NotificationsFragment;
import rs.highlande.app.diplomatici.features.profile.ProfileActivity;
import rs.highlande.app.diplomatici.features.profile.ProfileFragment;
import rs.highlande.app.diplomatici.features.profile.ProfileHelper;
import rs.highlande.app.diplomatici.features.tags.ViewAllTagsActivity;
import rs.highlande.app.diplomatici.features.timeline.LandscapeMediaActivity;
import rs.highlande.app.diplomatici.features.timeline.OnTimelineFragmentInteractionListener;
import rs.highlande.app.diplomatici.features.timeline.TimelineFragment;
import rs.highlande.app.diplomatici.features.timeline.interactions.FullScreenHelper;
import rs.highlande.app.diplomatici.features.timeline.interactions.PostBottomSheetHelper;
import rs.highlande.app.diplomatici.features.timeline.interactions.PostOverlayActionActivity;
...
import rs.highlande.app.diplomatici.models.Tag;
...
import rs.highlande.app.diplomatici.widgets.HLViewPagerNoScroll;
...
public class HomeActivity extends HLActivity implements View.OnClickListener, BasicInteractionListener,
	...
	private Integer previousPagerItem = null;
...
//		fullScreenListener.setToolbarExpHeight(toolbar.getHeight());
	...
	protected void onResume() {
		...
		if (notificationHelper != null)
			notificationHelper.updateNotifications(false);
...
//		DiplomaticiApp.profileFragmentVisible = currentPagerItem == HomeActivity.PAGER_ITEM_PROFILE;
	...
	private void configureBottomBar(final View bar) {
		if (bar != null) {
			...
			View l3 = bar.findViewById(R.id.bottom_chats);
			...
			overlay = bar.findViewById(R.id.disabled_overlay);
	...
	private void callToServer(CallType type, @Nullable FlagType flagType, String reportMessage) {
		...
		if (type != null) {
			...
			HLRequestTracker.getInstance(((DiplomaticiApp) getApplication())).handleCallResult(this, this, results);
		}
	}
	*/
...
//				intent.putExtra(Constants.EXTRA_PARAM_1, PAGER_ITEM_NOTIFICATIONS);
			...
			if (newIntent) {
				previousPagerItem = currentPagerItem;
				...
				if (intent.hasExtra(Constants.EXTRA_PARAM_2))
					fromLogin = intent.getBooleanExtra(Constants.EXTRA_PARAM_2, false);
...
//				if (weakGlobalSearch != null && weakGlobalSearch.get() != null) {
//					weakGlobalSearch.get().resetSearch();
//				}
			}
			else {
				if (intent.hasExtra(Constants.EXTRA_PARAM_1)) {
					...
					// INFO: 2019-09-24    adds new boolean param because Home does not know how to restore GlobSearch item after login
					if (intent.hasExtra(Constants.EXTRA_PARAM_2))
						fromLogin = intent.getBooleanExtra(Constants.EXTRA_PARAM_2, false);

//					if (currentPagerItem == PAGER_ITEM_PRONOTIFICATIONS)
...
//				fullScreenListener.goFullscreen(toolbar, bottomBar);
	...
	//	private WeakReference<GlobalSearchFragment> weakGlobalSearch;
	private class HLHomePagerAdapter extends FragmentStatePagerAdapter {
		...
		public Fragment getItem(int position) {
			switch (position) {
				case PAGER_ITEM_GLOBAL_SEARCH:
//					weakGlobalSearch = new WeakReference<>(GlobalSearchFragment.newInstance());
//					return weakGlobalSearch.get();
	...
	public void onPageSelected(int position) {
		...
		DiplomaticiApp.profileFragmentVisible = false;
		DiplomaticiApp.chatRoomsFragmentVisible = false;
...
//		Utils.closeKeyboard((weakHomeWebView != null && weakHomeWebView.get() != null) ? weakHomeWebView.get().getSearchView() : null);
		...
		switch (position) {
			case PAGER_ITEM_TIMELINE:
				...
				// INFO: 2019-09-24    adds new boolean param because Home does not know how to restore GlobSearch item after login
				switch (fromLogin ? PAGER_ITEM_GLOBAL_SEARCH : (previousPagerItem != null ? previousPagerItem : currentPagerItem)) {
					case PAGER_ITEM_PROFILE:
...
//						td4.reverseTransition(100);
			...
			case PAGER_ITEM_PROFILE:
				...
				DiplomaticiApp.profileFragmentVisible = true;
				NotificationAndRequestHelper.handleDotVisibility(notificationsDot, mUser.isValid());

				AnalyticsUtils.trackScreen(this, AnalyticsUtils.HOME_ME_PAGE);

				switch (previousPagerItem != null ? previousPagerItem : currentPagerItem) {
					case PAGER_ITEM_TIMELINE:
//						setBottomBar(false, position);
...
//				break;
			...
			case PAGER_ITEM_CHATS:
				...
				DiplomaticiApp.chatRoomsFragmentVisible = true;
