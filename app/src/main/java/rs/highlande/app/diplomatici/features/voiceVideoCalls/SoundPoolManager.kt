...
import rs.highlande.app.diplomatici.utilities.vibrateForCalls
...
class SoundPoolManager private constructor(context: Context,
    ...
    fun playRinging(context: Context) {
        if (usageType == VoiceVideoUsageType.OUTGOING) {
            ...
            if (DiplomaticiApp.canPlaySounds && mediaPlayer != null) {
                mediaPlayer!!.start()
                playingRingtone = true
            }

            if (DiplomaticiApp.canVibrate) vibrateForCalls(context)
        }
    }
