...
import rs.highlande.app.diplomatici.features.voiceVideoCalls.SDKTokenUtils;
...
//		onRestoreInstanceState(savedInstanceState != null ? savedInstanceState : getArguments());
	...
	public void onPause() {
		...
		// INFO: 2019-09-24    if set to false in modal screen showing this will stay false because home adapter has different logic
...
//					viewCreated = false;
	...
	public void onDotsClickUser(View dots) {
		if (profileType == ProfileHelper.ProfileType.FRIEND ||
			...
			// INFO: 2019-10-21    all users are virtually FRIENDS but they cannot remove each other
			if (userGeneric != null)
				remove.setVisible(/*userGeneric.isFriend()*/false);
			menu.show();
		}
	}
