...
import rs.highlande.app.diplomatici.models.enums.*
...
import rs.highlande.app.diplomatici.utilities.listeners.LottieCompositioListener
import rs.highlande.app.diplomatici.utilities.media.*
import java.io.File
...
class CreatePostPreviewActivity: HLActivity(), OnServerMessageReceivedListener, OnMissingConnectionListener,
    ...
    private fun initLayout() {
        ...
        if (mediaCaptureType == HLMediaType.AUDIO) {
            if (DiplomaticiApp.siriComposition != null)
                waveAnimation.setComposition(DiplomaticiApp.siriComposition)
            else {
                DiplomaticiApp.initLottieAnimation(this, object : LottieCompositioListener() {
    ...
    private fun attemptSendingPost(fromWiFiDialog: Boolean) {
        ...
        if (!checkConnectionAndShow(fromWiFiDialog)) {
            ...
            HLRequestTracker.getInstance(application as DiplomaticiApp).handleCallResult(this, this, results)
        }
    }
