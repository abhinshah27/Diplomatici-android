...
//		resultIntent.putExtra("some code", true);
		...
		if (data.containsKey(KEY_NOTIFICATION_TYPE)) {
			...
			if (Utils.isStringValid(code)) {
				...
				if (data.containsKey("notificationID"))
					notifId = data.get("notificationID");
				...
				switch (code) {
					...
					case "CHAT":
						if (data.containsKey("message")) {
							...
							createChatChannelAndSendNotification(this, Constants.NOTIFICATION_CHAT_ID, dataMessage, notifId, null);
						}
						break;
					...
					case "SOCIAL":
						if (data.containsKey("message")) {
							String dataMessage = data.get("message");
							sendNotification(Constants.NOTIFICATION_GENERIC_ID, dataMessage, notifId, null);
						}
						break;
				}
			}
	...
	private boolean checkFcmToken(Context context, String newToken) {
		...
		if (!DiplomaticiApp.fcmTokenSent || (Utils.isStringValid(newToken) && !fcm.equals(newToken))) {
	...
	private void sendNotification(int id, String msg, String notifId, Intent extra) {
		...
		if (mNotificationManager != null) {
			...
			if (extra == null) {
				...
				extra = new Intent(this, /*DiplomaticiApp.isForeground ? HomeActivity.class : SplashActivity.class*/ EmptyNotificationActivity.class);
				...
				extra.putExtra(Constants.FRAGMENT_KEY_CODE, Constants.FRAGMENT_PROFILE_CARD);
				extra.putExtra(Constants.KEY_NOTIFICATION_ID, notifId);
				extra.putExtra(Constants.KEY_NOTIFICATION_FOREGROUND, DiplomaticiApp.isForeground);
			}
...
//						.setLargeIcon(bpm)
							.setContentTitle((BuildConfig.FLAVOR == "demoDev") ? "Diplomatici" : getString(R.string.app_name))
...
//			int _id = (!Utils.isStringValid(id) ? (int) when : id.hashCode());
	...
	public static void createChatChannelAndSendNotification(Context context, int id, String msg, String notifId, Intent extra) {
		NotificationManager mNotificationManager = (NotificationManager)
		...
		if (mNotificationManager != null) {
			...
			if (extra == null) {
				extra = new Intent(context, /*DiplomaticiApp.isForeground ? HomeActivity.class : SplashActivity.class*/EmptyNotificationActivity.class);
				extra.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
			...
			String realMsg = msg.substring(colonIndex + 1);
            ...
            if (/*wantsGroup = */(Utils.hasNougat() && Utils.areStringsValid(userName, realMsg))) {
                if (Utils.hasPie())
                    style = new NotificationCompat.MessagingStyle(new Person.Builder().setName("Me").build())
                            .addMessage(realMsg, when, new Person.Builder().setName(userName).build());
                else
                    style = new NotificationCompat.MessagingStyle("Me")
                            .addMessage(new NotificationCompat.MessagingStyle.Message(realMsg.trim(), when, userName));
            }
            ...
            NotificationCompat.Builder mBuilder =
                    new NotificationCompat.Builder(context, context.getString(R.string.notif_channel_chat))
							...
							.setContentTitle((BuildConfig.FLAVOR == "demoDev") ? "Diplomatici" : context.getString(R.string.app_name))
