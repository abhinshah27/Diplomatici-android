...
import rs.highlande.app.diplomatici.models.enums.LegacyContactStatusEnum;
...
public class HLUser implements JsonHelper.JsonDeSerializer, RealmModel, RealmModelListener {
	...
	private String type = ProfileTypeEnum.NORMAL.toString();
...
//						getSelectedFeedFilters().remove(FeedFilterTypeEnum.getCallValue(Constants.INNER_CIRCLE_NAME));
	...
	public boolean isUserNormal() {
		return ProfileTypeEnum.toEnum(type) == ProfileTypeEnum.NORMAL;
	}
	public boolean isUserStaff() {
		return ProfileTypeEnum.toEnum(type) == ProfileTypeEnum.STAFF;
	}
	public boolean isUserTeacher() {
		return ProfileTypeEnum.toEnum(type) == ProfileTypeEnum.TEACHER;
	}

	public boolean canPost() {
		return isUserStaff() || isUserTeacher();
	}
