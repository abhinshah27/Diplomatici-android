...
package rs.highlande.app.diplomatici.features.timeline.interactions;
...
import rs.highlande.app.diplomatici.features.timeline.RealTimeCommunicationListener;
...
import rs.highlande.app.diplomatici.utilities.listeners.SlidingPanelHandler;
...
public class InteractionsViewerActivity extends HLActivity implements View.OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ivHearts.setImageResource((mPost.getHeartsLeft() != null && mPost.getHeartsLeft() > 0) ?
				R.drawable.transition_hearts_accent : R.drawable.transition_hearts);
		...
		if (resultCode == RESULT_OK) {
			switch (requestCode) {
				...
				case Constants.RESULT_REPLY_TO_COMMENT:
					DiplomaticiApp.pageIdComments = 1;
	...
	public int getLastPageId() {
		if (hUp)
			return DiplomaticiApp.pageIdHearts;
		else if (cUp)
			return DiplomaticiApp.pageIdComments;
		else if (sUp)
			return DiplomaticiApp.pageIdShares;
...
//					addPostToList(v, name);
	...
	private void setData(Realm realm, boolean background) {
		...
		switch (viewHitResId) {
			case R.id.button_hearts:
				if (DiplomaticiApp.pageIdHearts == 1) {
					...
					if (heartsList != null) {
						...
						if (!background) {
							heartsAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (DiplomaticiApp.pageIdHearts - 1)), newItemsCount
							);
						}
					}
				}
			...
			case R.id.button_comments:
				if (DiplomaticiApp.pageIdComments == 1) {
...
//							commentsList.addAll(HLPosts.getInstance().getVisibleCommentsForPost(mPostId));
						if (!background) {
							commentsAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (DiplomaticiApp.pageIdComments) - 1), newItemsCount
							);
						}
					}
				}
			...
			case R.id.button_shares:
				if (DiplomaticiApp.pageIdShares == 1) {
					...
					if (sharesList != null) {
						...
						if (!background) {
							sharesAdapter.notifyItemRangeInserted(
									(Constants.PAGINATION_AMOUNT * (DiplomaticiApp.pageIdShares - 1)), newItemsCount
							);
						}
					}
				}
...
//				cntSharesPost.setSelected(mPost.isYouDidShares());
	...
	private void call(boolean usePageId) {
		...
		switch (viewHitResId) {
			case R.id.button_hearts:
				type = InteractionPost.Type.HEARTS;
				if (usePageId) pageId = ++DiplomaticiApp.pageIdHearts;
				else pageId = -1;
				break;

			case R.id.button_comments:
				type = InteractionPost.Type.COMMENT;
				if (usePageId) pageId = ++DiplomaticiApp.pageIdComments;
				else pageId = -1;
				break;

			case R.id.button_shares:
				type = InteractionPost.Type.SHARE;
				if (usePageId) pageId = ++DiplomaticiApp.pageIdShares;
				else pageId = -1;
				break;
		}
	...
	private void handleInteractions() {
		if (interactionsList != null && llm != null) {
			...
			switch (viewHitResId) {
				case R.id.button_hearts:
					first = (DiplomaticiApp.pageIdHearts == 1);
//					if (mPost != null)
...
//						hUp = false;
					...
					break;
				case R.id.button_comments:
					first = (DiplomaticiApp.pageIdComments == 1);
//					if (mPost != null)
...
//						cUp = false;
					...
					break;
				case R.id.button_shares:
					first = (DiplomaticiApp.pageIdShares == 1);
//					if (mPost != null)
...
//						sUp = false;
	...
	private void handleTransitionDrawables(boolean first) {
		...
		@ColorRes int colorInactiveOrange = R.color.dipl_orange_alpha_med;
