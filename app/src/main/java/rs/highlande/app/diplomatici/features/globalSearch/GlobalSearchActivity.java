...
import rs.highlande.app.diplomatici.utilities.FragmentsUtils;
...
public class GlobalSearchActivity extends HLActivity implements View.OnClickListener,
	...
	private View bottomBar, bottomBar1, notificationDot, notificationDot1, notificationDotChat, notificationDotChat1;
	...
	public void onClick(View v) {
		if (!Utils.checkAndOpenLogin(this, mUser, HomeActivity.PAGER_ITEM_GLOBAL_SEARCH)) {
			switch (v.getId()) {
				case R.id.btn_close:
					finish();
					break;
	...
	private void configureBottomBar() {
		...
		if (bottomBar != null) {
			...
			View l3 = bottomBar.findViewById(R.id.bottom_chats);
			...
			View overlay = bottomBar.findViewById(R.id.disabled_overlay);
			...
			notificationDotChat = bottomBar.findViewById(R.id.notification_dot_chat);
		}
		...
		if (bottomBar1 != null) {
			...
			View overlay = bottomBar1.findViewById(R.id.disabled_overlay);
			View main = bottomBar1.findViewById(R.id.main_action_btn);
			if (mUser.canPost()) {
				overlay.setVisibility(View.GONE);
				main.setOnClickListener(this);
			}
		...
		if (viewId == R.id.main_action_btn) {
			...
			switch (viewId) {
				case R.id.bottom_profile:
					...
					break;
				case R.id.bottom_chats:
					pageId = HomeActivity.PAGER_ITEM_CHATS;
					break;
				case R.id.bottom_timeline:
...
//			FragmentsUtils.addFragmentNotNull(fragmentTransaction, fragment, target, requestCode);
	...
	public void goToProfile(@NonNull String userId, boolean isInterest) {
		if (mUser.getId().equals(userId)) {
			...
			ProfileActivity.openProfileCardFragment(this, type, userId, HomeActivity.PAGER_ITEM_GLOBAL_SEARCH);
		}
	}
