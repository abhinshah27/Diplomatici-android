...
import rs.highlande.app.diplomatici.connection.HLWebSocketAdapter;
import rs.highlande.app.diplomatici.connection.HLWebSocketClient;
import rs.highlande.app.diplomatici.connection.OnConnectionChangedListener;
...
import rs.highlande.app.diplomatici.features.voiceVideoCalls.CallsActivityNoService;
...
//		}
	...
	protected void onStart() {
		...
		HLSocketConnection connection = DiplomaticiApp.getSocketConnection();
		if (connection != null) {
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				if (rootContent != null) {
					...
					try {
						actionView.setTextColor(Utils.getColor(getResources(), R.color.colorAccent));
