...
public class HLRequestTracker {
    ...
    private void storeId(String body, String idOp, Context caller, boolean isChat) {
        ...
        try {
            if (!map.isEmpty()) {
                for (Map.Entry<String, Request> entry : map.entrySet()) {
                    ...
                    if (r.isValid())
                        DiplomaticiApp.getSocketConnection().sendMessage(r.body);
                    else
                        map.remove(r.id);
                }
            }
...
//				application.getHLContext().sendBroadcast(intent);
    ...
    public void handleCallResult(OnMissingConnectionListener listener, Context context, Object[] results, boolean isChat, boolean wantsProgress) {
        handleCallResult(listener, context, results, isChat, wantsProgress, 0);
    }

    public void handleCallResult(
    		OnMissingConnectionListener listener,
			Context context,
			Object[] results,
			boolean isChat,
        ...
        if (results != null) {
            storeId((String) results[1], (String) results[2], context, isChat);
            if (!((boolean) results[0]) && Utils.isContextValid(context)) {
            	if (context instanceof HLActivity)
					((HLActivity) context).closeProgress();

                // Attempts to regulate message visualization after reconnecting
                if (!DiplomaticiApp.justComeFromBackground && context instanceof HLActivity)
					((HLActivity) context).showAlert(R.string.error_data_connection);
            ...
            if (wantsProgress && context instanceof HLActivity) {
            	((HLActivity) context).setProgressMessage(message == 0 ? "Loading data" : context.getString(message));
            	((HLActivity) context).openProgress();
			}
        }
    }
	...
	public class Request {
		...
		public Request(String id, String body, Context caller) {
			this.id = id;
			this.body = body;
