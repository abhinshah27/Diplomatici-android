...
import rs.highlande.app.diplomatici.base.OnApplicationContextNeeded;
...
import rs.highlande.app.diplomatici.features.globalSearch.GlobalSearchActivity;
...
import rs.highlande.app.diplomatici.features.timeline.interactions.OnActionsResultFromBottom;
...
import rs.highlande.app.diplomatici.features.timeline.survey.SurveyVoteManager;
import rs.highlande.app.diplomatici.models.FeedFilter;
...
import rs.highlande.app.diplomatici.models.enums.FeedFilterTypeEnum;
...
import rs.highlande.app.diplomatici.utilities.helpers.LoadMoreResponseHandlerTask;
import rs.highlande.app.diplomatici.utilities.listeners.LoadMoreScrollListener;
...
public class TimelineFragment extends HLFragment implements View.OnClickListener,
		...
		SurveyVoteManager.SurveyVoteListener {
	...
	private WeakReference<SurveyBottomFragment> weakSurvey;
...
//			((HomeActivity) getActivity()).setPostDeletedListener(this);
	...
	public void onResume() {
		...
		if (usageType == FragmentUsageType.TIMELINE) {
			if (/*DiplomaticiApp.refreshAllowed*/false) {
...
//				getContext().unregisterReceiver(filterRcvr);
	...
	private void onRestoreInstanceState(Bundle arguments, Bundle savedInstanceState) {
		...
		if (filtersString != null && !filtersString.isEmpty()) {
			DiplomaticiApp.refreshAllowed = !filtersString.containsAll(mUser.getSelectedFeedFilters()) ||
					!mUser.getSelectedFeedFilters().containsAll(filtersString);

			if (DiplomaticiApp.refreshAllowed)
	...
	public void handleSuccessResponse(int operationId, JSONArray responseObject) {
		...
		if (Utils.isContextValid(context)) {
			switch (operationId) {
				case Constants.SERVER_OP_GET_TIMELINE_V2:
...
//									.execute(responseObject);
						...
						DiplomaticiApp.refreshAllowed = false;
						...
						boolean condition = operationId == Constants.SERVER_OP_GET_TIMELINE_V2 &&
				...
				case Constants.SERVER_OP_SETTINGS_IC_GET_SET_FEED:
					realm.executeTransaction(realm -> {
						...
						mListener.setLastPageID(1);
						DiplomaticiApp.refreshAllowed = true;
						mListener.setLastAdapterPosition(0);
...
//				LocalBroadcastManager.getInstance(getContext()).unregisterReceiver(realTimeHelper.getServerMessageReceiver());
	...
	private void callCircles() {
		...
		HLRequestTracker.getInstance(((DiplomaticiApp) getActivity().getApplication()))
	...
	private void populateFilters(Realm realm, HLUser user) {
		...
		if (Utils.isContextValid(getContext())) {
			if (!user.isActingAsInterest()) {
				...
				// INFO: 2019-10-02    remove circles from app.
...
//				}
	...
	// INFO: 2019-10-09    no filters
	private void handleFilterToggleVisibility() {
		filterToggle.setVisibility(/*conditionForLayout()*/false ? View.VISIBLE : View.GONE);
	}
...
//					instance.cleanRealmPostsNewSession(realm, true);
	...
	public void setWeakSurvey(WeakReference<SurveyBottomFragment> weakSurvey) {
		this.weakSurvey = weakSurvey;
	}

	@Override
	public void closeSurvey(String postId) {

		try {
			if (Utils.isStringValid(postId)) {
				postAdapter.notifyItemChanged(postAdapter.getItemPositionById(postId));
			}
		} catch (NoSuchElementException e) {
			e.printStackTrace();
		}

		weakSurvey.get().dismiss();
		weakSurvey.clear();
		weakSurvey = null;
	}
