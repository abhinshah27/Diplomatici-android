...
public class InnerCircleFeedAdapter extends RecyclerView.Adapter<FeedMemoryViewHolder> {
	...
	private static final int TYPE_SURVEY = 5;
	...
	public FeedMemoryViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
		...
		switch (viewType) {
			...
			case TYPE_SURVEY:
				view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_post_survey, parent, false);
				return new FeedMemorySurveyVH(view, fragment, mListener);
		}
...
//			setPlayBtn(((FeedMemoryVideoVH) holder).getPlayBtn());
	...
	/**
	 * Retrieves a Post position in {@code mValues} from the object id.
	 * @param postId The provided Post id.
	 * @return The position of the Post within {@code mValues}
	 */
	int getItemPositionById(String postId) throws NoSuchElementException {
		return Stream
				.of(mValues)
				.filter(posts -> posts.getId().equals(postId))
				.mapToInt(mValues::indexOf)
				.findFirst()
				.orElseThrow();
	}
