...
import rs.highlande.app.diplomatici.BuildConfig;
...
import rs.highlande.app.diplomatici.features.login.LoginActivity;
import rs.highlande.app.diplomatici.features.profile.UserDetailFragment;
import rs.highlande.app.diplomatici.features.settings.SettingsAccountFragment;
import rs.highlande.app.diplomatici.features.settings.SettingsICSingleCircleFragment;
import rs.highlande.app.diplomatici.features.settings.SettingsICTimelineFeedFragment;
import rs.highlande.app.diplomatici.features.settings.SettingsPrivacySelectionFragment;
import rs.highlande.app.diplomatici.features.settings.SettingsSecurityLegacy2StepFragment;
import rs.highlande.app.diplomatici.features.settings.SettingsSecurityLegacyContactTriggerFragment;
import rs.highlande.app.diplomatici.models.FamilyRelationship;
import rs.highlande.app.diplomatici.models.GenericUserFamilyRels;
import rs.highlande.app.diplomatici.models.HLCircle;
import rs.highlande.app.diplomatici.models.HLInterestAbout;
import rs.highlande.app.diplomatici.models.HLNotifications;
import rs.highlande.app.diplomatici.models.HLUser;
import rs.highlande.app.diplomatici.models.HLUserAboutMe;
import rs.highlande.app.diplomatici.models.HLUserAboutMeMore;
import rs.highlande.app.diplomatici.models.HLUserGeneric;
import rs.highlande.app.diplomatici.models.InteractionComment;
import rs.highlande.app.diplomatici.models.InteractionHeart;
import rs.highlande.app.diplomatici.models.InteractionPost;
import rs.highlande.app.diplomatici.models.InteractionShare;
import rs.highlande.app.diplomatici.models.LifeEvent;
import rs.highlande.app.diplomatici.models.MarketPlace;
import rs.highlande.app.diplomatici.models.MemoryMediaObject;
import rs.highlande.app.diplomatici.models.MoreInfoObject;
import rs.highlande.app.diplomatici.models.Notification;
import rs.highlande.app.diplomatici.models.Post;
import rs.highlande.app.diplomatici.models.PostSurvey;
import rs.highlande.app.diplomatici.models.ProfileContactToSend;
import rs.highlande.app.diplomatici.models.SettingsHelpElement;
import rs.highlande.app.diplomatici.models.WishEmail;
import rs.highlande.app.diplomatici.models.WishListElement;
import rs.highlande.app.diplomatici.models.enums.ActionTypeEnum;
import rs.highlande.app.diplomatici.models.enums.NotificationTypeEnum;
import rs.highlande.app.diplomatici.models.enums.PostTypeEnum;
import rs.highlande.app.diplomatici.models.enums.PrivacyPostVisibilityEnum;
import rs.highlande.app.diplomatici.models.enums.SearchTypeEnum;
import rs.highlande.app.diplomatici.models.enums.UnBlockUserEnum;
import rs.highlande.app.diplomatici.utilities.Constants;
import rs.highlande.app.diplomatici.utilities.LogUtils;
import rs.highlande.app.diplomatici.utilities.SharedPrefsUtils;
...
public class HLServerCalls {
	...
	public static Object[] subscribeToSocket(final Context context, final String userId, boolean isChat) {
		if (isChat) {
			if (!DiplomaticiApp.getSocketConnection().isConnectedChat())
				return resultsNotConnected();
		}
		else {
			if (!DiplomaticiApp.getSocketConnection().isConnected())
				return resultsNotConnected();
		}
		...
		if (Utils.isStringValid(userId)) {
			...
			if (isChat) {
				if (!DiplomaticiApp.subscribedToSocketChat) {
					DiplomaticiApp.getSocketConnection().sendMessageChat(jsonResult[0]);
					DiplomaticiApp.subscribedToSocketChat = true;
				}
			}
			else {
				if (!DiplomaticiApp.subscribedToSocket) {
					DiplomaticiApp.getSocketConnection().sendMessage(jsonResult[0]);
					DiplomaticiApp.subscribedToSocket = true;
				}
			}
		}
	...
	public static Object[] sendNotificationToken(String userId, String token) throws JSONException {
		...
		if (Utils.areStringsValid(userId, token)) {
			...
			if (!DiplomaticiApp.fcmTokenSent) {
	...
	public static Object[] getTimeline(@NonNull String userId, @NonNull String name, @Nullable String avatar,
		...
		if (Utils.isStringValid(userId)) {
			...
			jsonResult = buildMessageJson(Constants.SERVER_OP_GET_TIMELINE_V2,
...
//	}
	...
	public static Object[] voteSurvey(@NonNull String userId, @NonNull String postId, @NonNull PostSurvey survey)
			throws JSONException {
		...
		if (Utils.areStringsValid(userId, postId)) {
			...
			pairs.add(new Pair<>("userID", userId));
			pairs.add(new Pair<>("postID", postId));
			pairs.add(new Pair<>("answers", new JSONArray(survey.getAnswers())));
			list.add(pairs);

			jsonResult = buildMessageJson(Constants.SERVER_CODE_NAME, Constants.SERVER_OP_VOTE_SURVEY,
					Constants.SERVER_CODE_DB_NAME, Constants.SERVER_CODE_COLL_POSTS, list,
					"VOTE SURVEY call");
			DiplomaticiApp.getSocketConnection().sendMessage(jsonResult[0]);
		}
	 ...
	 * {@link rs.highlande.app.diplomatici.features.profile.ProfileHelper.ProfileType#FRIEND}.
	 * @param interestIdToFetch must always refer to the {@link rs.highlande.app.diplomatici.models.Interest} to be fetched.
...
//			return resultsNotConnected();
		...
		return new Object[] { DiplomaticiApp.getSocketConnection().isConnected(), jsonResult[0], jsonResult[1] };
	}
...
//			return resultsNotConnected();
	...
	public static Object[] updateInterestProfile(@NonNull String userId, @NonNull String interestId,
		...
		return updateProfile(userId, null, null, null, null, null, interestId, interestName,
				interestHeadline, interestAbout, moreInfo, null);
	}

	public static Object[] updateLanguages(@NonNull String userId, @NonNull List<String> languages)
			throws JSONException {

		return updateProfile(userId, Constants.PROFILE_CONTEXT_LANGUAGE, null, null, null, null, null, null,
		...
		if (Utils.isStringValid(userId)) {
			...
			if (!Utils.isStringValid(context) && interestAbout != null) {
				...
				if (Utils.isStringValid(context)) {
					...
					else if (context.equals(Constants.PROFILE_CONTEXT_LANGUAGE)) {
						JSONArray array = new JSONArray();
...
//			return resultsNotConnected();
	...
	public static Object[] setNotificationOpened(String userId, String notificationId) throws JSONException {
...
//			return resultsNotConnected();
		...
		if (Utils.areStringsValid(userId, notificationId)) {
			...
			jsonResult = buildMessageJson(Constants.SERVER_CODE_NAME, Constants.SERVER_OP_SET_NOTIF_OPENED,
		 			Constants.SERVER_CODE_DB_NAME, Constants.SERVER_CODE_COLL_USERS, list,
					"SET NOTIFICATION OPENED call");
			DiplomaticiApp.getSocketConnection().sendMessage(jsonResult[0]);
		}
...
//			return resultsNotConnected();
	...
	public static Object[] settingsOperationsOnFeedSettings(@NonNull String userId, @NonNull Bundle bundle,
		...
		if (!DiplomaticiApp.getSocketConnection().isConnected() && type == SettingsICTimelineFeedFragment.CallType.SET)
			return resultsNotConnected();
	...
	public static Object[] settingsOperationsOnPrivacy(@NonNull String userId, int idItem,
		...
		if (!DiplomaticiApp.getSocketConnection().isConnected() && type == SettingsPrivacySelectionFragment.CallType.SET)
			return resultsNotConnected();
...
//				if (Utils.isStringValid(queryString))
	...
	public static void doActionOnNotification(HLActivity activity, String notificationId, ActionTypeEnum type,
		...
		HLRequestTracker.getInstance(((DiplomaticiApp) activity.getApplication())).handleCallResult(connectionListener, activity, result);
	}
